class TetrisGame {
    ...
    initScene() {
        ...
        // Create camera - FIXED POSITIONING
        const aspectRatio = this.width / this.height;
        const viewSize = this.height * this.blockSize * 0.6;
        ...
        this.renderer.setSize(this.width * this.blockSize * 1.5, this.height * this.blockSize * 1.5);
    ...
    createBoardOutline() {
        ...
        // Center the board
        boardOutline.position.x = (this.width * this.blockSize) / 2 - this.blockSize / 2;
        boardOutline.position.y = (this.height * this.blockSize) / 2 - this.blockSize / 2;
        boardOutline.position.z = -this.blockSize/2;
    ...
    spawnTetromino() {
        ...
        const tetromino = new Tetromino(randomType, this.scene, this.blockSize);
    ...
    moveRowsDown(clearedRow) {
        ...
        this.gridBlocks.forEach(block => {
            if (block.y < clearedRow) {
                block.y++;
                block.mesh.position.y = block.y * this.blockSize;
            }
        });
    }
